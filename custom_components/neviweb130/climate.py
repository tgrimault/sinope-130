...
class Neviweb130Thermostat(ClimateEntity):
    ...
    def __init__(self, data, device_info, name, sku, firmware):
        ...
        self._snooze = 0
        self._activ = True
        self._attributes = [ATTR_WATTAGE, ATTR_KEYPAD, ATTR_BACKLIGHT,
                            ATTR_SYSTEM_MODE, ATTR_CYCLE, ATTR_DISPLAY2, ATTR_RSSI]
        _LOGGER.debug("Setting up %s: %s", self._name, device_info)

    def get_device_data(self, attributes):
    ...
    def update(self):
        if self._activ:
            """Get the latest data from Neviweb and update the state."""
            device_data, start = self.get_device_data(UPDATE_ATTRIBUTES + self._attributes)

            if "error" not in device_data:
                if "errorCode" not in device_data:
                    self.update_locals(device_data)
...
class Neviweb130WifiThermostat(Neviweb130Thermostat):
    ...
    def __init__(self, data, device_info, name, sku, firmware):
        ...
        self._load1 = 0
        self._attributes = [ATTR_CYCLE, ATTR_FLOOR_OUTPUT1, ATTR_WIFI_WATTAGE, ATTR_WIFI, ATTR_WIFI_KEYPAD,
...
class Neviweb130LowWifiThermostat(Neviweb130WifiThermostat):
